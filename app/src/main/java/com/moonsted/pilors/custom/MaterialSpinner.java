...
public class MaterialSpinner extends TextView {
  ...
  private void init(Context context, AttributeSet attrs) {
    ...
    left = right = bottom = top = resources.getDimensionPixelSize(R.dimen.landing_tv_msg_font);
    if (rtl) {
    ...
    if (!hideArrow) {
      arrowDrawable = getDrawable(context, R.drawable.ic_action_categories_down).mutate();
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      ...
      popupWindow.setBackgroundDrawable(getDrawable(context, R.drawable.ms__drop_down_shadow));
    } else {
      popupWindow.setBackgroundDrawable(getDrawable(context, R.drawable.ms__drop_down_shadow));
    }
  ...
  public <T> void setItems(@NonNull List<T> items) {
    ...
    adapter = new MaterialSpinnerAdapter<>(getContext(), items).setTextColor(dropDownTextColor);
